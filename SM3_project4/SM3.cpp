...
public:
    ...
    std::vector<uint8_t> hash(const uint8_t* message, size_t length) {
        ...
        for (int i = 0; i < 8; i++) {
            digest[i*4]   = (V[i] >> 24) & 0xFF; // 提取高8位
            digest[i*4+1] = (V[i] >> 16) & 0xFF; // 提取次高8位
            digest[i*4+2] = (V[i] >> 8)  & 0xFF; // 提取次低8位
            digest[i*4+3] = V[i] & 0xFF;         // 提取低8位
        }
...
private:
    ...
    std::vector<uint8_t> padding(const uint8_t* message, size_t length) {
        size_t bit_length = length * 8; // 计算消息的总比特长度
...
int main() {
    ...
    for (const auto& test : testCases) {
        auto digest = sm3.hash(test.input); // 计算哈希值
